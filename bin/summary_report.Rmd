...
params:
  ...
  sc2rf_results: "none"
  vois_results: "none"
  nextclade_percentN_spike: "none"
  num_mixed_sites: "none"
  cns_min_cov: "none"
  run_id: "none"
...
dt.nextclade_percentN_spike <- as.data.table(ldply(params$nextclade_percentN_spike, fread))
dt.num_mixed_sites <- as.data.table(ldply(params$num_mixed_sites, fread))
...
dt.overview <- merge(x=dt.overview, y=dt.nextclade_percentN_spike, by='sample', all.x=TRUE)
...
if(params$kraken_table != 'none'){
  col <- c("sample", '# total reads', '# filtered reads', "kraken2_ratio_sarscov2", "president_QC_pass", "president_identity", "president_percentN", "pangolin_lineage","scorpio_constellation", "nextclade_frameshifts","nextclade_privateMutations_status", 'nextclade_percentN_spike' ,'sc2rf_potential_recombinant', 'num_mixed_sites')
  col_names <- c("Sample", '# total reads', '# filtered reads', "Severe acute respiratory syndrome coronavirus 2 (ratio)", "Genome QC", "%Identity", "%N", "%N Spike", "Linage", "Scorpio call", "Frameshifts", "Private mutations QC", 'Potential recombinant', 'Mixed sites')
} else {
  col <- c("sample", '# total reads', '# filtered reads', "president_QC_pass", "president_identity", "president_percentN", "pangolin_lineage", "scorpio_constellation", "nextclade_frameshifts","nextclade_privateMutations_status", 'nextclade_percentN_spike', 'sc2rf_potential_recombinant', 'num_mixed_sites')
  col_names <- c("Sample", '# total reads', '# filtered reads', "Genome QC", "%Identity", "%N", "%N Spike", "Linage", "Scorpio call", "Frameshifts", "Private mutations QC", 'Potential recombinant', 'Mixed sites')
}
