...
if(params$kraken_table != 'none'){
  col <- c("sample", '# total reads', '# filtered reads', "kraken2_ratio_sarscov2", "president_QC_pass", "president_identity", "pangolin_lineage","scorpio_constellation", "nextclade_frameshifts","nextclade_privateMutations_status")
  col_names <- c("Sample", '# total reads', '# filtered reads', "Severe acute respiratory syndrome coronavirus 2 (ratio)", "Genome QC", "%N", "Linage", "Scorpio call", "Frameshifts", "Private mutations QC")
} else {
  col <- c("sample", '# total reads', '# filtered reads', "president_QC_pass", "president_identity", "pangolin_lineage","scorpio_constellation", "nextclade_frameshifts","nextclade_privateMutations_status")
  col_names <- c("Sample", '# total reads', '# filtered reads', "Genome QC", "%N", "Linage", "Scorpio call", "Frameshifts", "Private mutations QC")
}
...
datatable(dt.overview[, col, with=FALSE], 
      ...
      options = list(scrollX = TRUE, order = list(list(0, 'asc')), columnDefs = list(list(targets="_all", render = JS("function(data, type, row, meta) {return data === null ? 'n/a' : data;}")))),
      escape = FALSE)
```
...
- scorpio: `r params$scorpio_version` with constellation `r params$scorpio_constellations_version`

```{r lineage_assignment}
dt.pangolin_results$status <- ifelse(dt.pangolin_results$status == "passed_qc",
                                    cell_spec('PASS', background = "lightgreen", align = "right"),
                                    cell_spec('FAIL', background = "red", align = "right"))
datatable(dt.pangolin_results, 
...
Versions: nextclade `r params$nextclade_version` with dataset date `r params$nextclade_dataset_version`
```{r nextclade}
