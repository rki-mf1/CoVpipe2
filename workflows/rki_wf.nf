...
workflow rki_report_wf {
    ...
    main:
        ...
        // store valid genomes
        channel_tmp1 = president_valid.filter({ !it[0].contains("NK") }).map{it -> it[2]}
        ...
        channel_tmp2 = president_valid.filter({ !it[0].contains("NK") }).map{it -> tuple(it[0], it[2])}
            .splitText(by:100000000)
        ...
        channel_tmp3 = president_invalid.filter({ !it[0].contains("NK") }).map{it -> it[2]}
            .splitText(by:100000000)
        ...
        channel_tmp4 = president_invalid.filter({ !it[0].contains("NK") }).map{it -> tuple(it[0], it[2])}
            .splitText(by:100000000)
